...
public class RandomVerifyView extends View implements View.OnClickListener {
	...
	private int codeNum = 6;//验证码位数  4或6。。
	...
	public RandomVerifyView(Context context) {
		this(context,null);
	}
	
	public RandomVerifyView(Context context,AttributeSet attrs) {
		this(context,attrs,0);
	}
	
	public RandomVerifyView(Context context,AttributeSet attrs,int defStyleAttr)
	{
	...
	private void init(AttributeSet attrs) {
		TypedArray typedArray = getContext()
				.obtainStyledAttributes(attrs,R.styleable.RandomVerifyView);
		isNetCode = typedArray.getBoolean(R.styleable.RandomVerifyView_isNetCode,false);
	}
	...
	private void initView() {
		...
		textPaint.setTextSize(sp2px(30F));
	...
	private void createCodeBitmap() {
		...
		if (tempCode != null && tempCode.length() > 0) {
			...
			for (int i = 0;i < codeNum;i++) {
				...
				myCanvas.drawText(String.valueOf(tempCode.charAt(i)),
				                  i * charLength * 1.6f + 20 + horizontalOffset,mHeight * 2 / 3f,
				                  textPaint);
				myCanvas.restore();
			}
		}
		...
		for (Path path : mPaths) {
			linePaint.setARGB(255,mRandom.nextInt(200) + 20,mRandom.nextInt(200) + 20,
			                  mRandom.nextInt(200) + 20);
